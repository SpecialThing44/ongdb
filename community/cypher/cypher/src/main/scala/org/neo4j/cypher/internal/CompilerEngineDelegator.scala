...
//  private final val ILLEGAL_PLANNER_RUNTIME_COMBINATIONS: Set[(CypherPlanner, CypherRuntime)] =
//    Set((CypherPlanner.rule, CypherRuntime.compiled), (CypherPlanner.rule, CypherRuntime.slotted))
//  private final val ILLEGAL_PLANNER_VERSION_COMBINATIONS: Set[(CypherPlanner, CypherVersion)] =
//    Set((CypherPlanner.rule, CypherVersion.v3_3), (CypherPlanner.rule, CypherVersion.v3_4))
  ...
  def preParseQuery(queryText: String): PreParsedQuery = exceptionHandler.runSafely {
    ...
    val CypherStatementWithOptions(
      statement,
      offset,
      version,
      planner,
...
//      throw new InvalidArgumentException(s"Unsupported PLANNER - VERSION combination: ${pickedPlanner.name} - ${cypherVersion.name}")
  ...
  private def pick[O <: CypherOption](
      candidate: Option[O],
  ...
  private def getStatisticsDivergenceCalculator: StatsDivergenceCalculator = {
    val divergenceThreshold = getSetting(
      graph,
      config =>
    ...
    val targetThreshold = getSetting(
      graph,
    ...
    StatsDivergenceCalculator.divergenceCalculatorFor(
      divergenceAlgorithm,
      divergenceThreshold,
      targetThreshold,
      minReplanTime,
  ...
  private def getSetting[A](
      gds: GraphDatabaseQueryService,
      configLookup: Config => A,
      default: A
  ): A = gds match {
