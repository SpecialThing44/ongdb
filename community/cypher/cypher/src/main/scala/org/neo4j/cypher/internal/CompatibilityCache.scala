...
import org.neo4j.cypher.internal.compatibility.v3_4.runtime.{
  ...
  CommunityRuntimeContextCreator => CommunityRuntimeContextCreatorV3_4
}
...
final case class PlannerSpec_v3_4(
...
class CommunityCompatibilityFactory(
    ...
    (spec.planner, spec.runtime) match {
      case (CypherPlanner.rule, _) =>
        throw new InvalidArgumentException(
          "The rule planner is no longer a valid planner option in ONgDB 1.0. If you need to use it, please select compatibility mode Cypher 3.1"
        )
      case _ =>
        Compatibility(
          config,
