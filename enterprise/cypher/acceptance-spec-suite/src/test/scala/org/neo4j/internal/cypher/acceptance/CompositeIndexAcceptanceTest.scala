...
class CompositeIndexAcceptanceTest extends ExecutionEngineFunSuite with CypherComparisonSupport {
  ...
  test("should use composite index when all predicates are present") {
    ...
    val result = executeWith(
      Configs.Interpreted,
      "MATCH (n:User) WHERE n.lastname = 'Soap' AND n.firstname = 'Joe' RETURN n",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
        plan should useOperators("NodeIndexSeek")
      }, Configs.DefaultInterpreted)
    )
  ...
  test("should not use composite index when not all predicates are present") {
    ...
    val result = executeWith(
      Configs.All,
      "MATCH (n:User) WHERE n.firstname = 'Jake' RETURN n",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
  ...
  ignore("should use composite index with combined equality and existence predicates") { // Enable once we support index scan on composite indexes
    ...
    val result = executeWith(
      Configs.Interpreted,
      "MATCH (n:User) WHERE exists(n.lastname) AND n.firstname = 'Jake' RETURN n",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
  ...
  test("should be able to update composite index when only one property has changed") {
    ...
    val result = executeWith(
      Configs.Interpreted,
      "MATCH (n:Person) where n.firstname = 'Joe' and n.lastname = 'Bloggs' RETURN n",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
        plan should useOperators("NodeIndexSeek")
      }, Configs.DefaultInterpreted)
  ...
  test(
    ...
    executeWith(Configs.DefaultInterpreted, "WITH RANGE(0,10) AS num CREATE (:Person {id:num})") // ensure label cardinality favors index
    executeWith(Configs.DefaultInterpreted, "CREATE (n:Person {firstname:'Joe', lastname:'Soap'})")
    graph.createIndex("Person", "firstname")
    graph.createIndex("Person", "firstname", "lastname")
    executeWith(
      Configs.Interpreted,
  ...
  test("should handle missing properties when populating index") {
    ...
    val result = executeWith(
      Configs.Interpreted,
      "MATCH (n:L {foo: 42, bar: 1337, baz: 1980}) RETURN count(n)",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
  ...
  test("should not use range queries against a composite index") {
    ...
    val result = executeWith(
      Configs.Interpreted,
      "match (n:X) where n.p1 = 1 AND n.p2 > 0 return n;",
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
  ...
  test("should not use composite index for range, prefix, contains and exists predicates") {
    ...
    val n = createLabeledNode(
      Map("name" -> "joe", "surname" -> "soap", "age" -> 25, "active" -> true),
      "User"
    )
    ...
    Seq(
      (
        Configs.Interpreted,
        "n.name = 'joe' AND n.surname = 'soap' AND n.age = 25 AND n.active = true",
        true
      ), // all equality
      (
        Configs.Interpreted,
        "n.surname = 'soap' AND n.age = 25 AND n.active = true AND n.name = 'joe'",
        true
      ), // different order
      (
        Configs.Interpreted,
      ...
      (
        Configs.Interpreted,
        "n.name = 'joe' AND n.surname STARTS WITH 's' AND n.age = 25 AND n.active = true",
        false
      ), // prefix
      (
        Configs.Interpreted,
        "n.name = 'joe' AND n.surname ENDS WITH 'p' AND n.age = 25 AND n.active = true",
        false
      ), // suffix
      (
        Configs.Interpreted,
        "n.name >= 'i' AND n.surname = 'soap' AND n.age = 25 AND n.active = true",
        false
      ), // inequality first
      (
        Configs.Interpreted,
        "n.name STARTS WITH 'j' AND n.surname = 'soap' AND n.age = 25 AND n.active = true",
        false
      ), // prefix first
      (
        Configs.Interpreted,
        "n.name CONTAINS 'j' AND n.surname = 'soap' AND n.age = 25 AND n.active = true",
        false
      ), // contains first
      (
        Configs.Interpreted,
        ...
        false
      ) // combination: equality, prefix, inequality, exists()
    ).foreach {
        ...
        val result = try {
          if (valid)
            ...
            executeWith(
              ...
              planComparisonStrategy = ComparePlansWithAssertion((plan) => {
                //THEN
                plan shouldNot useOperatorWithText(
                  "NodeIndexSeek",
                  ":User(name,surname,age,active)"
                )
              })
            )
        } catch {
  ...
  test("should use composite index with spatial") {
    // Given
    val n1 = createLabeledNode(
      Map(
        "name" -> "Joe",
        "city" -> Values.pointValue(CoordinateReferenceSystem.Cartesian, 1.2, 5.6).asObjectCopy()
      ),
      "User"
    )
    createLabeledNode(
      Map(
        "name" -> "Joe",
        "city" -> Values.pointValue(CoordinateReferenceSystem.Cartesian, 1.2, 3.4).asObjectCopy()
      ),
      "User"
    )
    ...
    val resultNoIndex = executeWith(Configs.Interpreted - Configs.DefaultInterpreted, query)

    graph.createIndex("User", "name", "city")
    val resultIndex = executeWith(
      Configs.Interpreted - Configs.DefaultInterpreted,
      query,
      planComparisonStrategy = ComparePlansWithAssertion((plan) => {
        //THEN
        plan should useOperators("NodeIndexSeek")
  ...
  test("should use composite index with temporal") {
    // Given
    val n1 = createLabeledNode(
      Map("date" -> LocalDate.of(1991, 10, 18), "time" -> LocalTime.of(21, 22, 0)),
      "Label"
    )
    val n2 = createLabeledNode(
      Map(
        "date" -> LocalDate.of(1991, 10, 18),
        "time" -> OffsetTime.of(21, 22, 0, 0, ZoneOffset.of("+00:00"))
      ),
      "Label"
    )
    val n3 =
      createLabeledNode(Map("date" -> "1991-10-18", "time" -> LocalTime.of(21, 22, 0)), "Label")
    val n4 = createLabeledNode(
      Map("date" -> LocalDate.of(1991, 10, 18).toEpochDay, "time" -> LocalTime.of(21, 22, 0)),
      "Label"
    )
  ...
  test("should use composite index with duration") {
    // Given
    val n1 = createLabeledNode(
      Map("name" -> "Neo", "result" -> DurationValue.duration(0, 0, 1800, 0).asObject()),
      "Runner"
    )
  ...
  case class haveIndexes(expectedIndexes: String*) extends Matcher[GraphDatabaseQueryService] {
    def apply(graph: GraphDatabaseQueryService): MatchResult = {
      graph.inTx {
        ...
        MatchResult(
          result,
          s"Expected graph to have indexes ${expectedIndexes
            .mkString(", ")}, but it was ${indexNames.mkString(", ")}",
