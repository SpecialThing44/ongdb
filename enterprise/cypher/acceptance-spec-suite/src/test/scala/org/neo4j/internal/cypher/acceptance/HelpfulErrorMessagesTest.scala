...
class HelpfulErrorMessagesTest extends ExecutionEngineFunSuite with CypherComparisonSupport {
  ...
  test(
    "should provide sensible error message when omitting colon before relationship type on create"
  ) {
  ...
  test(
    "should provide sensible error message when omitting colon before relationship type on merge"
  ) {
    failWithError(
      Configs.AbsolutelyAll,
  ...
  test("should provide sensible error message for 3.4 rule planner") {
    intercept[Exception](graph.execute("CYPHER 3.4 planner=rule RETURN 1")).getMessage should be(
      "Unsupported PLANNER - VERSION combination: rule - 3.4"
    )
  }
  ...
  test("should provide sensible error message for rule planner and slotted") {
    intercept[Exception](graph.execute("CYPHER planner=rule runtime=slotted RETURN 1")).getMessage should be(
      "Unsupported PLANNER - RUNTIME combination: rule - slotted"
    )
  }
  ...
  test("should provide sensible error message for invalid regex syntax together with index") {
    ...
    failWithError(
      Configs.AbsolutelyAll - Configs.AllRulePlanners - Configs.Compiled ,
      "MATCH (x:Person) WHERE x.text =~ '*xxx*yyy*' RETURN x.text",
      List("Invalid Regex:")
    )
  }
  ...
  test("should provide sensible error message for START in newer runtimes") {
    ...
    failWithError(
      Configs.SlottedInterpreted + Configs.Compiled,
      query,
      Seq("The given query is not currently supported in the selected runtime")
    )
  }
