...
import org.neo4j.cypher.{GraphDatabaseTestSupport, GraphIcing}
...
import org.neo4j.internal.cypher.acceptance.ExecutionEngineHelper.createEngine
...
trait EnterpriseExecutionEngineTestSupport extends CypherTestSupport with ExecutionEngineHelper {
...
object ExecutionEngineHelper {
  ...
  def createEngine(graphDatabaseCypherService: GraphDatabaseQueryService, logProvider: LogProvider = NullLogProvider.getInstance()): ExecutionEngine = {
    ...
    val compilerFactory =
      new EnterpriseCompilerFactory(communityCompilerFactory, graphDatabaseCypherService, kernelMonitors, nullLogProvider,
        cypherConfig.toCypherPlannerConfiguration(config), cypherConfig.toCypherRuntimeConfiguration)
    //val config = resolver.resolveDependency(classOf[Config])
