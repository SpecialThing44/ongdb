...
class ExpressionAcceptanceTest extends ExecutionEngineFunSuite with CypherComparisonSupport {
  ...
  test("should handle map projection with property selectors") {
    ...
    val result = executeWith(Configs.Interpreted , "MATCH (n) RETURN n{.foo,.bar,.baz}")
  ...
  test("should handle map projection with property selectors and identifier selector") {
    ...
    val result = executeWith(Configs.Interpreted , "WITH 42 as x MATCH (n) RETURN n{.foo,.bar,x}")
  ...
  test("should use the map identifier as the alias for return items") {
    ...
    val result = executeWith(Configs.Interpreted , "MATCH (n) RETURN n{.foo,.bar}")
  ...
  test("map projection with all-properties selector") {
    ...
    val result = executeWith(Configs.Interpreted , "MATCH (n) RETURN n{.*}")
  ...
  test("returning all properties of a node and adds other selectors") {
    ...
    val result = executeWith(Configs.Interpreted , "MATCH (n) RETURN n{.*, .baz}")
  ...
  test("returning all properties of a node and overwrites some with other selectors") {
    ...
    val result = executeWith(Configs.Interpreted , "MATCH (n) RETURN n{.*, bar:'apatisk'}")
  ...
  test("projecting from a null identifier produces a null value") {
    ...
    val result = executeWith(Configs.Interpreted , "OPTIONAL MATCH (n) RETURN n{.foo, .bar}")
  ...
  test("graph projections with aggregation") {
    ...
    val result = executeWith(Configs.Interpreted , """MATCH (actor:Actor)-->(movie:Movie)
  ...
  test("not(), when right of a =, should give a helpful error message") {
    ...
    val config =  Configs.AbsolutelyAll
  ...
  test("NOT(), when right of a =, should give a helpful error message") {
    ...
    val config =  Configs.AbsolutelyAll- Configs.Version3_1  - Configs.AllRulePlanners
