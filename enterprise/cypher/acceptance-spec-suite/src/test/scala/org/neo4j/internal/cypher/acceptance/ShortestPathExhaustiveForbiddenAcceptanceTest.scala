...
class ShortestPathExhaustiveForbiddenAcceptanceTest
  ...
  test("should fail at run time when using the shortest path fallback") {
    // when
    failWithError(
      Configs.AbsolutelyAll - Configs.AllRulePlanners,
  ...
  override protected def initTest(): Unit = {
    ...
    0 to dMax foreach { row =>
      0 to dMax foreach { col =>
        val name = s"$row$col"
        val node = createLabeledNode(
          Map("name" -> name, "row" -> row, "col" -> col),
          s"CELL$row$col",
          s"ROW$row",
          s"COL$col"
        )
