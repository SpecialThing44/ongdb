...
class MutatingIntegrationTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with CypherComparisonSupport {
  ...
  test("create a single node") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "create (a)")
  ...
  test("create a single node with props and return it") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "create (a {name : 'Andres'}) return a.name")
  ...
  test("start with a node and create a new node with the same properties") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "match (a) where id(a) = 0 with a create (b {age : a.age * 2}) return b.age")
  ...
  test("create two nodes and a relationship between them") {
    val result = executeWith(Configs.DefaultInterpreted, "create (a), (b), (a)-[r:REL]->(b)")

    assertStats(result, nodesCreated = 2, relationshipsCreated = 1)
  }

  test("create one node and dumpToString") {
    val result = executeWith(Configs.DefaultInterpreted, "create (a {name:'Cypher'})")
  ...
  test("deletes single node") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "match (a) where id(a) = 0 delete a")
  ...
  test("multiple deletes should not break anything") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "match (a), (b) where id(a) = 0 AND id(b) IN [1, 2, 3] delete a")
    assertStats(result, nodesDeleted = 1)
  ...
  test("deletes all relationships") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "match (a) where id(a) = 0 match (a)-[r]->() delete r")
  ...
  test("create multiple relationships in one query") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "create (n) with n MATCH (x) WHERE id(x) IN [0, 1, 2] create (n)-[:REL]->(x)")
    assertStats(result,
      nodesCreated = 1,
  ...
  test("set a property to a collection") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "MATCH (n) with collect(n.name) as names create (m {name : names}) RETURN m.name")
    assertStats(result,
      propertiesWritten = 1,
      nodesCreated = 1
    )
  ...
  test("set a property to an empty collection") {
    val result = executeWith(Configs.DefaultInterpreted, "create (n {x : []}) return n.x")
    assertStats(result,
      propertiesWritten = 1,
      nodesCreated = 1
    )
  ...
  test("create node from map values") {
    val result = executeWith(Configs.DefaultInterpreted, "create (n {a}) return n.age, n.name", params = Map("a" -> Map("name" -> "Andres", "age" -> 66)))
  ...
  test("create rel from map values") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "match (a), (b) where id(a) = 0 AND id(b) = 1 create (a)-[r:REL {param}]->(b) return r.name, r.age", params = Map("param" -> Map("name" -> "Andres", "age" -> 66)))
  ...
  test("match and delete") {
    ...
    failWithError(Configs.DefaultInterpreted + Configs.Procs, "match (n) where id(n) = 0 match (n)-[r:HATES]->() delete n,r", List(msg))
  }

  test("delete and return") {
    val a = createNode()

    val result = executeWith(Configs.DefaultInterpreted, "match (n) where id(n) = 0 delete n return n")
  ...
  test("create multiple nodes") {
    ...
    val result = executeWith(Configs.DefaultInterpreted, "unwind {params} as m create (x) set x = m ", params = Map("params" -> maps))
  ...
  test("not allowed to create multiple nodes with parameter list") {
    ...
    failWithError(Configs.DefaultInterpreted + Configs.Procs, "create ({params})", params = Map("params" -> maps), message = errorMessages)
  }
  ...
  test("fail to create from two iterables") {
    ...
    failWithError(Configs.DefaultInterpreted + Configs.Procs, query, message = errorMessages, params = Map("params1" -> maps1, "params2" -> maps2))
  }
  ...
  test("first read then write") {
    ...
    executeWith(Configs.DefaultInterpreted, "match (root) where id(root) = 0 match (root)-->(other) create (new {name:other.name}), (root)-[:REL]->(new)")
  ...
  test("create node and rel in foreach") {
    executeWith(Configs.DefaultInterpreted, """
  ...
  test("delete optionals") {
    ...
    executeWith(Configs.DefaultInterpreted, """match (n) optional match (n)-[r]-() delete n,r""")
  ...
  test("delete path") {
    ...
    executeWith(Configs.DefaultInterpreted, """match (n) where id(n) = 0 match p=(n)-->() delete p""")
  ...
  test("string literals should not be mistaken for variables") {
    ...
    executeWith(Configs.DefaultInterpreted, "EXPLAIN create (tag1 {name:'tag2'}), (tag2 {name:'tag1'}) return [tag1,tag2] as tags")
  ...
  test("create node from map with array value from java") {
    ...
    assertStats(executeWith(Configs.DefaultInterpreted, q, params = Map("param"->map)), nodesCreated = 1, propertiesWritten = 1)
  ...
  test("full path in one create") {
    createNode()
    createNode()
    val result = executeWith(Configs.DefaultInterpreted, "match (a), (b) where id(a) = 0 AND id(b) = 1 create (a)-[:KNOWS]->()-[:LOVES]->(b)")
  ...
  test("delete and delete again") {
    createNode()
    val result = executeWith(Configs.DefaultInterpreted, "match (a) where id(a) = 0 delete a foreach( x in [1] | delete a)")
  ...
  test("create with parameters is not ok when variable already exists") {
    ...
    failWithError(Configs.AbsolutelyAll, "create (a) with a create (a {name:\"Foo\"})-[:BAR]->()", errorMessages)
  }

  test("failure_only_fails_inner_transaction") {
    val tx = graph.beginTransaction( Type.explicit, AnonymousContext.write() )
    try {
      executeWith(Configs.DefaultInterpreted, "match (a) where id(a) = {id} set a.foo = 'bar' return a", params = Map("id"->"0"))
    } catch {
  ...
  test("create two rels in one command should work") {
    val result = executeWith(Configs.DefaultInterpreted, "create (a{name:'a'})-[:test]->(b), (a)-[:test2]->(c)")
  ...
  test("cant set properties after node is already created") {
    val errorMessages = List("The variable is already declared in this context", "It already exists in this context")
    failWithError(Configs.AbsolutelyAll, "create (a)-[:test]->(b), (a {name:'a'})-[:test2]->(c)", errorMessages)
  }

  test("can create anonymous nodes inside foreach") {
    createNode()
    val result = executeWith(Configs.DefaultInterpreted, "match (me) where id(me) = 0 foreach (i in range(1,10) | create (me)-[:FRIEND]->())")
  ...
  test("should be able to use external variables inside foreach") {
    createNode()
    val result = executeWith(Configs.DefaultInterpreted, "match (a), (b) where id(a) = 0 AND id(b) = 0 foreach(x in [b] | create (x)-[:FOO]->(a)) ")

    result.toList shouldBe empty
  }

  test("should be able to create node with labels") {
    val result = executeWith(Configs.DefaultInterpreted, "create (n:FOO:BAR) return labels(n) as l")
  ...
  test("complete graph") {
    val result =
      executeWith(Configs.DefaultInterpreted, """CREATE (center { count:0 })
  ...
  test("for each applied to null should never execute") {
    val result = executeWith(Configs.DefaultInterpreted, "foreach(x in null| create ())")
